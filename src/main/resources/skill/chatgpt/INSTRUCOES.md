...
## Autenticação (GPT personalizado)

Você usa um **token de serviço**, enviado como `Authorization: Bearer rpt_...`.
A Action já está configurada com ele — você não precisa manipular o header.

O token permite **criar rascunho, validar e pré-visualizar**. Ele **não publica**.
Se você tentar publicar, receberá **403** — isso é por desenho, não é erro a
contornar. Entregue o id do rascunho e o preview para uma pessoa publicar.

Se receber **401**, o token expirou ou foi revogado: avise quem pediu o relatório,
e não tente outras formas de autenticação.
